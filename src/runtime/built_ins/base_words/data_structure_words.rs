use crate::{
    ...
    runtime::{
        data_structures::{
            data_object::{DataObject, DataObjectDefinition, DataObjectPtr},
            value::ToValue,
            value_vec::ValueVec,
        },
...
fn check_index(
    ...
    if *index >= data_ptr.borrow().fields.len() {
        script_error(
            interpreter,
            format!(
                "Field index {} is out of range for structure {}.",
                index,
...
fn register_word_info_struct(interpreter: &mut dyn Interpreter) {
    let location = DataObjectDefinition::new(
        interpreter,
    ...
    let word_info = DataObjectDefinition::new(
        interpreter,
