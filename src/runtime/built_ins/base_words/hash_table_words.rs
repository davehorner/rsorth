use crate::{
    ...
    runtime::{
        data_structures::{value::ToValue, value_hash::ValueHash},
...
fn word_hash_table_insert(interpreter: &mut dyn Interpreter) -> error::Result<()> {
    ...
    if key.is_float() {
        script_error_str(
            interpreter,
            "Hash table keys can not be floating point numbers.",
        )?;
    }
