use crate::{
    add_native_word,
    lang::{
        code::{/*pretty_print_code,*/ ByteCode, Op},
        compilation::{CodeConstructor, CodeConstructorList, process_source_from_tokens},
        source_buffer::SourceLocation,
        tokenizing::{NumberType, Token, TokenList, tokenize_from_file, tokenize_from_source},
    },
    location_here,
    runtime::{
        ...
        interpreter::{
            CallItem, CallStack, CodeManagement, Ffi, Interpreter, InterpreterStack,
            ThreadManagement, ValueStack, VariableList, WordHandler, WordHandlerInfo,
            WordManagement,
        },
    },
};
...
impl Interpreter for SorthInterpreter {
    fn add_search_path(&mut self, path: &str) -> error::Result<()> {
        if let Err(err) = metadata(path) {
            script_error(
                self,
                format!("Could not append search path {}: {}.", path, err),
            )?;
        }
...
impl SorthInterpreter {
    ...
    fn read_variable(&mut self) -> error::Result<()> {
        ...
        let value = {
            if (index as usize) >= self.variables.len() {
                script_error(
                    self,
                    format!("Read index {} out of range of variable set.", index),
                )?;
            }
    ...
    fn write_variable(&mut self) -> error::Result<()> {
        ...
        if (index as usize) >= self.variables.len() {
            script_error(
                self,
                format!("Write index {} out of range of variable set.", index),
            )?;
        }
    ...
    fn execute_value(&mut self, value: &Value) -> error::Result<()> {
        let location = if let Some(location) = &self.current_location {
            location.clone()
        } else {
            location_here!()
        };
        ...
        match value {
            Value::String(word_name) => self.execute_word_named(&location, word_name),

            Value::Token(token) => match token {
                Token::Word(location, word_name) => self.execute_word_named(location, word_name),

                _ => script_error(self, format!("Token {} is not executable.", token)),
            },
    ...
    fn absolute_index(&self, pc: usize, relative_index: &Value) -> error::Result<usize> {
        ...
        // instruction.
        let absolute = if relative_index.is_numeric() {
            (pc as i64 + relative_index.get_int_val()) as usize
        } else {
            return script_error(self, format!("Invalid loop exit index {}.", relative_index));
        };
...
impl WordManagement for SorthInterpreter {
    ...
    fn execute_word_handler(
        ...
        self.call_stack.push(CallItem::new(
            word_handler_info.name.clone(),
            location.clone(),
        ));
