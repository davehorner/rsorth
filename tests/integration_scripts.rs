...
use sorth::runtime::built_ins::{
    base_words::register_base_words, ffi_words::register_ffi_words, io_words::register_io_words,
...
use sorth::runtime::interpreter::{CodeManagement, Interpreter};
use std::fs;
...
fn assert_00_test_words_output(output: &str) {
    assert!(
        ...
        "Missing 'Hello world!' in output"
    );
    assert!(
        output.contains("Goodbye all."),
        "Missing 'Goodbye all.' in output"
    );
    assert!(
        output.contains("The 10 is greater than 0!"),
        "Missing 'The 10 is greater than 0!' in output"
    );
    assert!(
        output.contains("What about failed ifs?"),
        "Missing 'What about failed ifs?' in output"
    );
    assert!(
        output.contains("All done."),
        "Missing 'All done.' in output"
    );
...
fn assert_01_test_loops_output(output: &str) {
    for n in (0..=9).rev() {
        assert!(
            output.contains(&format!("Looping until: {}", n)),
            "Missing 'Looping until: {}'",
            n
        );
    }
    for n in (0..=9).rev() {
        assert!(
            output.contains(&format!("Looping while: {}", n)),
            "Missing 'Looping while: {}'",
            n
        );
    }
...
fn test_00_test_words_lib() {
    ...
    interpreter
        .add_search_path(std_path.to_str().unwrap())
        .unwrap();
    interpreter
        .process_source_file(manifest_path("std.f").to_str().unwrap())
        .unwrap();
    let script = fs::read_to_string(manifest_path("tests/00_test_words.f")).unwrap();
    let result = interpreter.process_source(
        manifest_path("tests/00_test_words.f").to_str().unwrap(),
        &script,
    );
...
fn test_01_test_loops_lib() {
    ...
    let result = interpreter.process_source(
        manifest_path("tests/01_test_loops.f").to_str().unwrap(),
        &script,
    );
    assert!(result.is_ok(), "Script failed: {:?}", result.err());
